/******************************************************************************
 Script     : AnonimizadorDB.sql
 Descripción: Script base completo del sistema Anónima CGR.
              Crea el esquema completo: tablas, secuencias, triggers, índices,
              stored procedures, tabla de configuración y datos iniciales.
 Autor      : Ruiz
 Fecha      : 2026
 ...
 INSTRUCCIONES DE EJECUCIÓN:
   ...
   2. Ejecutar este script completo con F5 (Run Script)
   3. Ejecutar luego los packages por separado:
      - Package.sql        → pkg_auth_anonimizador
      - pkg_wizard_anon.sql → pkg_wizard_anon
******************************************************************************/


-- =============================================
-- LIMPIEZA SEGURA — PROCEDURES
-- =============================================
...
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE WIZARD_SESSION_FILES CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE WIZARD_RESULT_FILES CASCADE CONSTRAINTS';  EXCEPTION WHEN OTHERS THEN NULL; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE ANONIMIZADOR_CONFIG CASCADE CONSTRAINTS';  EXCEPTION WHEN OTHERS THEN NULL; END;
/
...
-- Archivos temporales del wizard de anonimización (persiste el archivo entre pasos)
...
-- Resultados anonimizados listos para descarga (TTL: 2 horas por defecto)
...
-- Configuración centralizada del sistema (reemplaza constantes hardcodeadas)
...
-- ⚠️  Reemplazar el hash antes de ejecutar en un ambiente nuevo
...
-- Configuración centralizada — URL de la API y retención de archivos temporales
INSERT INTO ANONIMIZADOR_CONFIG (config_key, config_value, description)
VALUES ('API_BASE_URL', 'http://127.0.0.1:5255',
        'URL base de la API .NET. Cambiar a la URL de GCP en producción.');

INSERT INTO ANONIMIZADOR_CONFIG (config_key, config_value, description)
VALUES ('SESSION_RETENTION_H', '2',
...
BEGIN
    OPEN p_ResultSet FOR
        SELECT u.UserId, u.Username, u.PasswordHash,
               u.IsActive, u.FullName, r.RoleName
        FROM   USERS u
...
BEGIN
    OPEN p_ResultSet FOR
        SELECT d.DocumentId, d.OriginalFileName, d.ContentType,
               d.FileSizeKB, d.UploadedBy,
               s.Name AS Status, d.CreatedAt, d.UpdatedAt
        FROM   DOCUMENTS d
...
/*
 * SP_METRICS_SUMMARY — integrado desde migration 04_fix_metrics_sps.sql
 * Agrega ThisMonth y ActiveUsers al resumen original.
 */
...
/*
 * SP_METRICS_DOCUMENTS_BY_MONTH — integrado desde migration 04_fix_metrics_sps.sql
 * Agrega MonthNumber, Year, límite 6 meses y NLS español.
 */
...
BEGIN
    OPEN p_ResultSet FOR
        SELECT COALESCE(UploadedBy, 'Desconocido') AS Usuario,
               COUNT(*)                             AS Total
        FROM   DOCUMENTS
        GROUP BY UploadedBy
        ORDER BY Total DESC;
...
-- VERIFICACIÓN FINAL
-- =============================================
SELECT 'PROCESS_STATUS'     AS Objeto, COUNT(*) AS Registros FROM PROCESS_STATUS   UNION ALL
SELECT 'ROLES',                         COUNT(*)              FROM ROLES             UNION ALL
SELECT 'USERS',                         COUNT(*)              FROM USERS             UNION ALL
SELECT 'ANONIMIZADOR_CONFIG',           COUNT(*)              FROM ANONIMIZADOR_CONFIG UNION ALL
SELECT 'DOCUMENTS',                     COUNT(*)              FROM DOCUMENTS         UNION ALL
SELECT 'DOCUMENT_VERSIONS',             COUNT(*)              FROM DOCUMENT_VERSIONS UNION ALL
SELECT 'ANONYMIZED_FIELDS',             COUNT(*)              FROM ANONYMIZED_FIELDS UNION ALL
SELECT 'WIZARD_SESSION_FILES',          COUNT(*)              FROM WIZARD_SESSION_FILES UNION ALL
SELECT 'WIZARD_RESULT_FILES',           COUNT(*)              FROM WIZARD_RESULT_FILES;